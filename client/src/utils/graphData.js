...
export const montInObject = items => {
  let months = items.reduce(
    (unique, item) =>
      unique.includes(new Date(item.date).getMonth())
        ? unique
        : [...unique, new Date(item.date).getMonth()],
    []
  );
  ...
  months.forEach(month => {
    ...
    payments.forEach(payment => {
      if (new Date(payment.dateInterestPaid).getMonth() === month)
        if (!paymentInarray(payment, group_Payment_in_EachMonth))
          group_Payment_in_EachMonth.push(payment);
