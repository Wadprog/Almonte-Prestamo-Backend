...
router.post('/', async (req, res) => {
	...
	try {
		...
		if (!plan) return res.status(404).json({ msg: `Este plan ${req.body.plan} no existe` });
		...
		for (var step = 1; step < steps + 1; step++) {
			var newdate = moment(loan.date).add(step * interval, 'days').format('l');
...
router.post('/due/:id', async (req, res) => {
	try {
		...
		if (loan_.status) return res.status(400).json({ msg: `Pago rechazado prestamo pagado` });
